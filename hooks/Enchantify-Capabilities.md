...
*Version: 16.5.0 — Support Guild, Wider Research & Fresh Memory Rails*
...
- **Living Book Pages:** Enchantify now treats every interaction as a Page: every page has a purpose, a player invitation, a closure condition, proof the Book keeps, and a contextual tool posture. `mechanics/pages.md` defines the grammar. `scripts/page-contract.py` chooses the current Page, and `scene-contract.py` embeds it before scene generation so smaller models ask: what page are we on, what does it want, what proof will it leave behind, and which tools are appropriate right now? Core Page types: Slice of Life, Conflict, Enchantment, Wonder Compass, Letter, Anchor, Rest, Dr. Vellum / Body Marginalia, Dr. Inkrest / Difficult, Archive, and Bleed.
...
- **Bleed Feedback Loop:** The Bleed is now bidirectional. Game state feeds the newspaper; published articles write `logs/bleed-ripples.jsonl`; `story-context.py` and `scene-director.py` read those ripples as public interpretation, rumor pressure, market offers, practical nudges, and social weather. The Bleed does not rewrite canon by fiat — it changes what the Academy thinks happened, and that belief becomes playable pressure.
...
| Gimble of the Errata Registry | Goblin Bursar / Errata Registry clerk; brass BOUND stamp; smells uncategorized transactions like smoke | Can over-classify and mistake emotional avoidance for a filing problem | Reads ledger chart/log and Actual Budget when configured; gives Money Weather, Alchemical Audits, Tiny Leak notes, Adventure Permission Slips, and transaction/category prompts without shame |
...
| `scripts/npc-research.py` | Living-world research / scheduled notes | NPC researches a topic from their Unwritten Interest and delivers findings. Uses local OpenClaw gateway (`NPC_RESEARCH_MODEL`, timeout) instead of spawning `openclaw agent`; `--model-smoke` verifies the route. Selection now includes any world-register NPC with Belief ≥ 8 and no cooldown, weighted by Belief, Unwritten Interest fit, relationship, core-cast bonus, and recent-research penalties. Character lookup canonicalizes titles, parentheticals, aliases, and compact lore rows. `--npc "Name"` forces an NPC and bypasses cooldown intentionally; `--dry-run` previews; `--no-print` skips CUPS; `--no-icloud` skips Notes; `--no-letter-image` skips Draw Things ornament; `--preview-letter memory/npc-research/[slug]-[date].md` renders an existing note as a styled letter without delivery. Writes to `memory/npc-research/[slug]-[date].md`, `memory/npc-research/letters/[slug]-[date].html`, optional `letter-images/[slug]-[date].png`, and a PostScript fallback for printing when needed. Physical printing is attempted for delivered research unless `--no-print` is used; the script submits with explicit `lp -d <current default>` so stale `~/.cups/lpoptions` defaults cannot block non-core NPC letters. Telegram and iCloud delivery have explicit timeouts. Queues tick-queue narrative seed. Deducts 3 Belief. 72-hour cooldown except forced runs; cache is `config/npc-research-cache.json`. |
...
| `scripts/scene-contract.py` | Before and after active scene drafting | Emits MODE / DRAMA_BUDGET / PAGE / LONG_MEMORY / QUIET_LIFE / grounding / continuity / classroom / tool-packet / choice-contract constraints. `--json` exposes current scene construction rails for Story-Field Journal. `--validate-scene` checks grounding, mode fit, class compression, Page fit, and Rule-of-Three-related expectations before delivery. |
...
- Promoted Gimble from readiness to live Actual Budget / SimpleFIN support: daily sync, auto-categorization, fresh balances, recent transactions, Money Weather, Mission Control ledger data, Heartbeat finance data, and Telegram delivery all flow through the local Actual API bridge.
