...
*Version: 16.7.0 — Storybook, Press, Memory & Offer Cadence*
...
**Model:** Brain is the configured OpenClaw Enchantify route, commonly `claude-sonnet-4-6` for active play when available. OpenClaw also has Codex routes for `openai-codex/gpt-5.4-mini`, `openai-codex/gpt-5.4`, `openai-codex/gpt-5.5-instant`, and `openai-codex/gpt-5.5`; high-value synthesis jobs such as Support Guild, Press/Abundance, and market research use the `openclaw/gpt55` route for intelligence. Set in `~/.openclaw/openclaw.json`.
...
- **Living Book Pages:** Enchantify now treats every interaction as a Page: every page has a purpose, a player invitation, a closure condition, proof the Book keeps, and a contextual tool posture. `mechanics/pages.md` defines the grammar. `scripts/page-contract.py` chooses the current Page, and `scene-contract.py` embeds it before scene generation so smaller models ask: what page are we on, what does it want, what proof will it leave behind, and which tools are appropriate right now? Core Page types: Dorm, Slice of Life, Conflict, Enchantment, Wonder Compass, Letter, Anchor, Rest, Dr. Vellum / Body Marginalia, Dr. Inkrest / Difficult, Gimble / Ledger, Bellkeeper / Today's Page, Penny Blackletter / Press, Storybook, Archive, and Bleed.
- **Belief / Narrative Weight Engine:** Belief is the game's atomic narrative mass, not just a player stat. Every meaningful person, talisman, object, location, thread, anchor, faction, bargain, and artifact can carry Belief in `lore/world-register.md`; higher Belief means the world selects it more often, notices it harder, gives it more agency, and lets it bend scenes. The player competes and allies with NPCs, chapter talismans, threads, fae, places, and objects by earning, investing, defending, and attacking Belief. Compass Runs and Enchantments turn real attention into Belief; `write-entity.py`, `belief-attack.py`, `tick.py`, `world-pulse.py`, and `narrative_sim.py` turn Belief into narrative weight. This is the living strategy layer: what receives attention becomes more real, and what loses Belief fades toward the margins.
...
- **Bleed Feedback Loop:** The Bleed is now bidirectional. Game state feeds the newspaper; published articles write `logs/bleed-ripples.jsonl`; `story-context.py` and `scene-director.py` read those ripples as public interpretation, rumor pressure, market offers, practical nudges, and social weather. `scripts/bleed-memory.py` also distills the latest issue into daily overwritten per-entity Bleed notices inside `memory/entities/bleed-awareness.json` and rendered entity cards, so characters can reference today's issue as fresh gossip, public pressure, or something they personally noticed. The Bleed does not rewrite canon by fiat — it changes what the Academy thinks happened, and that belief becomes playable pressure.
- **Entity Memory Rail:** World entities now have deterministic persistent memory through `scripts/entity_memory.py`. Simulation actions, Belief consequences, outreach sent/replies, delivered live scenes, and latest Bleed notices write/read `logs/entity-memory.jsonl`, `memory/entities/bleed-awareness.json`, and readable `memory/entities/*.md` cards. `story-context.py` injects relevant compact `ENTITY_MEMORY` before scenes so characters can remember BJ's replies, reference past interactions and Bleed stories, avoid repeating offscreen moves, and let continuity shape behavior.
- **World Simulation:** A background cron runs every 4 hours to evolve world state. It now uses a weighted living-world simulation: high-Belief entities act more often, but low-Belief entities still retain a real chance to move. NPCs, talismans, threads, anchors, arcs, and other weighted entities can all influence offscreen pressure. Selected NPCs now carry a character dossier from `lore/characters.md` into their actor profile — role, personality, quirks, faults, goals, beliefs, therapeutic/support spine, and Unwritten Interest — so their chosen action and visible prose trace should come from who they are, not just their register row. The mechanical layer has only three primitives: `take_action`, `invest_belief`, and `attack_belief`. Every selected action costs Belief, including slice-of-life traces. Player-facing summaries must read as bespoke narrative vignettes, never action-label reports. Daily-life traces avoid recent repeated wording and stale stock phrases. Actor selection includes global recent-actor penalties and freshness bonuses so the same characters and same action shapes do not monopolize the world. Each run can still trigger NPC research, but that research is bounded so it cannot freeze the pulse.
...
- **The Empathy Engine:** Monitors player wellness via heartbeat (including biometrics from Apple Watch/Health), fuel logs, and FoodData Central nutrition lookup when available, then responds with narrative care — never clinical guilt. Dr. Elowen Vellum now functions as a precise Literary Elf longevity physician/dietician, connecting logged meals, recovery, movement, and current longevity research to usable everyday counsel. Nightly intelligence detects low steps, poor sleep, and mood dips; writes tick-queue interventions in the Labyrinth's voice.
- **Support Faculty Loop:** Dr. Selene Inkrest, Dr. Elowen Vellum, and Gimble now have a separate daily support system distinct from ordinary NPC research. `scripts/support-faculty.py` handles Inkrest check-ins, explicit mood logging, Vellum briefs, synthesis, and support research. Inkrest asks for one-word emotional weather morning, midday, and evening; each prompt leaves `players/bj-inkrest-pending.json` so a short Telegram reply like `tired` routes to `support-faculty.py inkrest-route` instead of becoming an accidental narrative prompt. Explicit phrases like `Log for Dr Inkrest: awake and productive`, `Inkrest: anxious`, or `Mood: tired but okay` are also accepted even without a pending prompt. Logged replies go to `players/bj-inkrest-log.jsonl`. Vellum sends one practical midday body/fuel hinge as **Dr. Vellum's Refectory Marginalia**: a personal note from her desk that still includes the current fuel/steps/watch data, cross-domain insights, and one body-support hinge. Synthesis correlates mood words, steps, fuel visibility, recent briefs, pending prompt state, ledger state, and deterministic cross-domain insight rows into `players/bj-support-memory.json`. Weekly independent support briefs and experiments are saved under `memory/support-faculty/research/` and may be delivered through Telegram.
- **Support Guild Meeting:** `scripts/support-guild.py` runs a daily one-call faculty council for Dr. Vellum, Dr. Inkrest, Gimble, Penny, Bellkeeper Elian Quill, and Professor Bastion Goldweaver. It reads fuel, mood, health, ledger, calendar/day-shape, press/market work, heartbeat, current support memory, and `scripts/support_insights.py` cross-domain observations, then produces practical cross-domain insight, one small next experiment, and a storybook-journal artifact. It archives the full meeting under `memory/support-faculty/guild/`, logs to `logs/support-faculty/guild.jsonl`, updates `players/bj-support-memory.json`, and can send the Telegram summary plus a PDF/document artifact.
- **Gimble / Ledger Page:** Added Gimble of the Errata Registry, a Goblin Bursar of Small Abundances. He gives shame-free finance support through `scripts/ledger-faculty.py`, `players/bj-ledger-chart.md`, `players/bj-ledger-log.jsonl`, Actual Budget, and SimpleFIN-backed bank sync. Ledger Pages produce plain budget notes, weekly budget reviews, recurring-charge notes, and Adventure Permission Slips. A daily sync can refresh Actual Budget, auto-categorize transactions through configured rules/categories, update Gimble's memory, and send **Gimble's Daily Actual Sync** through Telegram as a personal message from him, not a system digest. The note keeps balances, categorized/uncategorized transactions, recent category samples, and cross-domain insight rows clear and readable. Gimble may categorize, summarize, and suggest next actions, but must not handle bank login, move money, shame debt, give tax/legal certainty, or make risky investment recommendations.
- **Bellkeeper / Today's Page:** Added Bellkeeper Elian Quill, Registrar of Hours, as the first proactive-office support character. `scripts/bellkeeper.py` reads `hooks/enchantify_schedule.ics`, Apple Calendar summaries when available, HEARTBEAT, and support-character traces to write a humane Today’s Page card under `memory/support-faculty/bellkeeper/` and log it to `players/bj-bellkeeper-log.jsonl`. `config/proactive-offices.json` is the expandable registry for future offices such as Postmaster, Cartographer, Lamplighter, or Registrar. The Bellkeeper may propose calendar/reminder support, Compass windows, transitions, and evening scraps, but does not create or modify calendar/reminder items without explicit permission.
- **Doctor Pages:** The support faculty now have first-class Page contracts. `body_marginalia` (Dr. Vellum) translates fuel, movement, sleep, BP/labs, supplements, exercise, and longevity research into one BJ-sized experiment, safety flag, or doctor/pharmacist question. `difficult` (Dr. Inkrest) is explicitly her Difficult Page: narrative therapy, grounding, daydream/image work, and reauthoring with consent-first rails. Both are low-intrusion Pages with chart-backed artifacts and no shame, diagnosis, forced catharsis, or generic wellness copy.
- **The Book of You / Storybook Page:** `scripts/storybook.py daily bj --send` gathers the day's proof into a nightly illustrated chapter: scene ledger, diary, Heartbeat, fuel, mood, ledger, Support Guild, Bleed ripples, simulation vignettes, outreach, anchors, Compass/Enchantment traces, and a deliberate mix of play images plus hourly Draw Things images. It writes Markdown to `memory/storybook/daily/`, renders a parchment/story-journal HTML page to `memory/storybook/html/`, renders a PDF to `memory/storybook/pdf/` using `wkhtmltopdf`, headless Google Chrome, or macOS `cupsfilter` when available, updates `memory/storybook/index.md`, logs to `logs/storybook.jsonl`, and can push the PDF through Telegram as a document. `storybook.py monthly` binds a month of daily pages into a collected PDF with intro/theme/highlight material. This is the player's ongoing illustrated life-story, distinct from The Bleed's public newspaper voice.
- **Penny Blackletter / Press Page:** Penny Blackletter is now a full Belief-bearing character and the Editor-in-Chief of The Bleed, Margins Desk. She owns ethical in-world marketing and content curation for The Wonder Compass, the Doobaleedoos $1 Patreon "one-dollar door," and free open-source Enchantify. `scripts/market-research.py daily` now runs the shared Listening Desk first, gathering public audience/platform signals for Penny and Professor Goldweaver under `memory/publishing/market-research/`. `scripts/penny-press.py scan bj` reads the daily Storybook, scene ledger, Bleed issue, support-guild notes, recent images, NPC research, Wonder Compass excerpts, market research, and capability-feature lines, then drafts a Press Packet under `memory/publishing/press-packets/` with Shimmering Fragments, public-safe social drafts, open-source Enchantify dispatches, Patreon invitations, Wonder Compass book notes, product seeds, privacy labels, and structured candidates in `memory/publishing/candidates/`. Penny also has an interactive studio: `scripts/penny-press.py proposal bj` creates a lightweight afternoon desk menu, while `scripts/penny-press.py make bj --kind <type> --topic "<topic>"` creates on-demand briefs for X posts/threads, Instagram carousels, TikTok carousels, YouTube Shorts, long-form YouTube concepts/scripts, Reddit posts/comments, Patreon posts, and content menus. `scripts/penny-press.py autonomous bj --send` lets Penny draft a small mission-shaped bundle on her own and send the consent tray to Telegram. Briefs live under `memory/publishing/briefs/`, proposals under `memory/publishing/proposals/`, editorial doctrine under `memory/publishing/editorial-strategy.md`, pending approvals under `memory/publishing/consent-queue.json`, and publication/activity memory under `memory/publishing/social-ledger.jsonl`. Autonomy is currently Level 1: draft autonomously and ask BJ for consent; no auto-posting yet.
- **Professor Bastion Goldweaver / Applied Abundance:** Goldweaver is the flamboyant entrepreneurial professor who turns Penny's editorial stream, market research, Storybook proof, Wonder Compass philosophy, and Doobaleedoos goals into ethical products, Patreon strategy, merchandise ideas, offer maps, and tiny next actions. `scripts/goldwater.py` manages product/revenue briefs. `scripts/press-abundance.py` convenes Penny + Goldweaver in one synthesis call, reads the shared Listening Desk, and writes a storybook-journal PDF for Telegram so strategy, content, and monetization stay in-world, consent-gated, and connected to actual output.

**NPC Research:** During the world simulation and through scheduled research paths, any sufficiently present world-register NPC can research topics from their Unwritten Interests and deliver findings to the player (local file, styled HTML letter artifact, optional Draw Things ornament, Telegram, iCloud Notes, and physical letters for core NPCs). It is no longer restricted to a tiny core/relationship list: Belief, cooldown, freshness, Unwritten Interest, relationship, and recent research history shape the weighted choice. `logs/npc-research-history.jsonl` and `memory/npc-research/history/[character].md` prevent exact-topic loops; the next prompt receives prior dispatches plus a rotating required angle so the character must find a fresh subtopic, case study, practical experiment, or surprising mechanism. Costs them Belief. Generation uses the local OpenClaw gateway with a bounded timeout and deterministic fallback, not long-lived `openclaw agent` processes. Physical letters render as character-specific storybook stationery, with a PostScript fallback if HTML/PDF conversion is unavailable. Woven into the next session as a tick-queue seed.
...
- **Story-Field Journal:** Formerly Mission Control. Live HTML folio (`hooks/mission-control.html`) showing threads, full world-register entities, talismans, forecast, heartbeat, schedule, cron job status, app actions, Bleed issues, Page Contract, Scene Director slate/scene-construction rails, narrative stewardship, images, anchors, inventory, and pact action history. Thread cards show live status from world-register Active Threads Notes; the Entities tab groups every register section, including Whisper Register entries. Auto-regenerated on every pulse run. Run manually: `python3 scripts/mission-control.py`.
...
- **Return Grace / Anti-Guilt Rule:** The game must never punish a depressed or overwhelmed player for absence. `session-entry.py` emits RETURN_TONE on every opening: welcome them back with relief, warmth, and curiosity; never imply failure, neglect, disappointment, debt, or guilt. Consequences from time away are allowed only as world texture and invitations: changed rooms, waiting objects, letters, rumors, and optional tiny Compass/Enchantment re-entry. Dusk Thorn no longer rises from ordinary 3-day gaps; only long silence can become marginal pressure, and even then the first beat is welcome.
- **Scene Delivery Pipeline:** Active-play scenes flow through a gated, multi-modal stack: `story-context.py` (long-memory braid + Bleed ripples) → `page-contract.py` (living-book Page purpose/artifact/tool posture) → `scene-contract.py` (mode/drama/grounding/scene-construction contract) → `mechanics-preflight.py` (mechanics gate) → `scene-preflight.py` (speaker verification) → `scene-choices.py` (Rule-of-Three validation) → `run-live-scene.py` (canonical entry) → `play_scene.py` → `scene_packet_builder.py` → `scene_conductor.py`. `scene-contract.py` now includes Scene Fullness rails so smaller models cannot pass off postcard replies as live play: normal scenes target roughly 650-1150 words before choices by mode/Page, enforce an absolute floor, require multiple substantial paragraphs, and demand polished choice invitations rather than bare command fragments. Page tool posture chooses contextual affordances (image, voice, lights, Spotify, MusicGen, printer, wallpaper, app actions) instead of one global all-or-nothing tool mode. Character-focused images now consult the self-forming visual canon in `lore/character-visuals.json`; `scripts/character-visuals.py` promotes core, high-Belief, and thread-active characters into stable visual identities with recurring signatures, palettes, silhouettes, and avoid notes. The next image prompt auto-refreshes the visual bible when `lore/characters.md` or `lore/world-register.md` changes. All delivered scenes are ledgered in `logs/scene-ledger/` as JSONL, including scene contract data.
...
**Offer cadence:** At Belief 60 or below, Compass Runs are no longer emergency-only. If the player has not completed one today, `mechanics_state.py`, `mechanics-preflight.py`, and `scene-contract.py` should surface one formal daily Compass Run as an optional path when it fits. The offer must route through `scripts/compass-run.py start`; completion still requires the real West souvenir and cannot be narrated in prose alone.
...
**Offer cadence:** At Belief 60 or below, Enchantments should surface more regularly as real script-backed options, not hallucinated flavor. `mechanics_state.py` permits up to two routine Enchantment offers per day when no Enchantment has completed today, while `scene-contract.py` chooses the actual opportunity from class context, concrete objects/clues, Nothing pressure, unresolved NPC traces, or quiet-life wonder. If accepted, run `scripts/enchantment.py start`; success still requires real photo or detailed description proof before `complete`.
...
**Canonical script:** `scripts/book-jump.py` is now the deterministic Book Jump state machine. It owns the active book, anchor, intention, guide, depth, return count, Nothing degradation, souvenir due flag, archive artifact, Belief cost/reward, and return. The Labyrinth supplies prose; the script supplies rails.
...
**Return:** Equally vivid. Text fragments trail behind. Debrief with the class. Return requires the script's `return` command and a real souvenir sentence unless it is an emergency exit.

**Script commands:** `start`, `advance`, `stabilize`, `status`, `return`, `cancel`, `list`.

**Belief:** Starting a jump costs 1 Belief. Returning with a valid souvenir grants +6 Belief; returning from *The Wonder Compass* memory jump grants +18 and counts as a Compass-adjacent breakthrough.
...
| Professor Wellend Thickets | Brilliant, cooperative, insightful Riddlewind head; riddles and tomes; studies mysteries, rabbit holes, chaos magick forums, sigils, and belief becoming consensus | Secretive, manipulative, deceptive; Duskthorn-aligned | Plants riddles, orchestrates conflict under cooperative cover, reveals partial truths, recruits through curiosity |
...
| Professor Archibald Permancer | Meticulous Historical Fiction professor; artifacts and scrolls; studies obituaries, museums, plaques, vanished foodways, and historical cooking | Secretly obsessed with dark arts | Surfaces old records, dangerous precedents, and historical parallels |
...
| Serenity Brown | Tidecrest friend; carefree, spontaneous, joyful; researches The Elder Scrolls Online lore, builds, guilds, housing, crafting, MMO sociology, and classic Bungie Marathon terminals / AI mythology | Distractible, chases shiny threads | Adds joyful sideways energy, compares Academy factions to MMO systems, brings game-community insight into social and magical structures |
...
| Gimble of the Errata Registry | Goblin Bursar / Errata Registry clerk; brass BOUND stamp; smells uncategorized transactions like smoke | Can over-classify and mistake emotional avoidance for a filing problem | Reads ledger chart/log and Actual Budget when configured; gives plain budget notes, weekly budget reviews, recurring-charge notes, Adventure Permission Slips, and transaction/category prompts without shame |
| Bellkeeper Elian Quill | Book Fae Registrar of Hours; brass spectacles, green waistcoat, pocket watch that ticks before overcommitment | Can over-prepare and make open time feel too carefully arranged | Reads calendar/schedule/Heartbeat/support traces; writes Today's Page cards, transition supports, Compass-window suggestions, and evening scrap prompts without nagging |
...
| Penny Blackletter | Riddlewind student editor; red pencil, PUBLIC-SAFE stamp, headline-bright voice; social media and marketing specialist for Wonder Compass, Doobaleedoos Patreon, and open-source Enchantify | Can over-headline tenderness or mistake urgency for importance | Creates Press Packets, public-safe drafts, Wonder Compass field assignments, OSS Enchantify explainers, Patreon invitations, X threads, Instagram/TikTok carousels, YouTube Short and long-form scripts, Reddit-safe posts/comments, privacy labels, and product seeds; drafts only, never auto-posts |
| Professor Bastion Goldweaver | Flamboyant Professor of Applied Abundance; velvet waistcoat, gold fountain pen, ethical-scheme ledger; product and Patreon strategist | Can become dazzled by possibility and mistake a spectacle for a shippable offer | Works with Penny and the Listening Desk to turn Enchantify/Wonder Compass output into ethical product briefs, Patreon nurture, merchandise ideas, launch experiments, offer maps, and one tiny revenue action |
...
**NPC Research:**
NPC research is a living-world pressure and scheduled support path: characters research topics from their Unwritten Interests and deliver findings to the player as in-world letters, Telegram notes, files, and sometimes printed artifacts. The selection pool is deliberately broad now, so the same two characters do not dominate the feature.
...
- **NPC selection:** Weighted by Belief, Unwritten Interest fit, relationship, core-cast bonus, and freshness. Eligibility: NPC row in `lore/world-register.md`, Belief ≥ 8, not on 72-hour cooldown. There is no longer a hard core-cast or relationship ≥ 25 gate. Recent research from the same NPC is penalized so the roster breathes.
- **Unwritten Interest lookup:** `scripts/npc-research.py` canonicalizes names across parentheticals, titles, aliases, and compact character entries so `lore/world-register.md` rows resolve back to `lore/characters.md` reliably. All Belief ≥ 8 NPCs should have a usable Unwritten Interest. Current interest texture includes witchcraft and magick, chaos magick, Bungie Marathon, The Elder Scrolls Online, restaurants, cooking, ethnic cooking, historical cooking, weather witchcraft, folk remedies, temple food, and local civic folklore.
...
- **Delivery:** Always written to `memory/npc-research/[npc-slug]-[date].md`. Each delivered note also produces a styled letter artifact in `memory/npc-research/letters/[npc-slug]-[date].html`; optional Draw Things ornaments live in `memory/npc-research/letter-images/`. Physical printing is attempted for delivered research unless `--no-print` is used. If HTML-to-PDF conversion is unavailable, a character-styled PostScript fallback is generated and printed instead. Telegram delivery is on by default. iCloud Notes ("Labyrinth" folder via osascript) is on by default unless `--no-icloud` is used. Delivery subprocesses have explicit timeouts.
...
- **Cooldown:** 72 hours per NPC. Cache stored in `config/npc-research-cache.json`. `--npc "Name"` is an intentional override path and bypasses cooldown for forced tests or deliberate letters.
...
- **Return grace:** Do not reduce player Belief simply because they have not opened the book for a while. Absence is life happening, not failure. On return, the Academy may show changed objects, waiting letters, rumors, softened rooms, and offscreen thread movement, but the first emotional fact is welcome. Longer gaps should invite a tiny Compass Run or Enchantment as warm re-entry, never as debt.
...
**Offer thresholds:**
- **Belief ≤ 60:** Recovery range. If no Compass Run is completed today, offer one formal daily Compass Run as an option when it fits.
- **Belief ≤ 60:** Up to two routine Enchantment opportunities per day may surface when scene/class/object/clue/quiet-life context supports a real script-backed spell.
- **Belief ≤ 25:** Critical range. Compass/Enchantment offers should become warmer and more prominent, but still optional and never guilt-framed.
...
Reality Wagers govern wild, impossible, scene-breaking, or reality-rewriting actions. The Labyrinth almost never says "you cannot"; it asks what the player is willing to spend and lets the page answer strangely.

**Procedure:** classify the action, spend Belief up front with `update-player.py`, roll with `roll-dice.py`, narrate the result, then apply any failure cost if stakes call for it.

| Wager | Cost | Difficulty | Examples |
|---|---:|---|---|
| Wild physical stunt | 1 | standard/dramatic | leap from balcony, climb impossible shelves |
| Non-Enchantment impossibility | 2 | dramatic | try to fly, bend a room rule |
| Direct narrative edit | 3 | dramatic/desperate | force a clue, declare a scene detail |
| Major reality rewrite | 5 | desperate | become headmaster, erase an enemy, skip an arc |
| Brute-force the Nothing | 5 | desperate | attack the Nothing directly without proof or Compass work |

**Rule:** Playful wildness feeds wonder. Arbitrary reality-breaking feeds the Nothing. Repeated attempts to erase consequence, ignore relationships, or flatten the scene should manifest as coherence loss — generic voices, forgotten names, faded detail, flattened choices — and offer repair through Enchantment, Compass Run, apology/relationship repair, or a smaller wager that engages the scene.

Reality Wagers cannot replace Enchantment proof, complete Compass Runs, permanently defeat the Nothing, or overwrite an NPC's inner life.
...
**Page contract:** `python3 scripts/page-contract.py [player] [--mode MODE] [--page-type slice_of_life|conflict|enchantment|wonder_compass|letter|anchor|rest|body_marginalia|difficult|archive|bleed]`
...
**Scene contract:** `python3 scripts/scene-contract.py [player] [--mode slice|school-life|dorm|arc|mystery|aftermath|compass|enchantment] [--page-type dorm|...]`
...
| `scripts/book-jump.py` | Labyrinth (formal Book Jumping) | Deterministic Book Jump state machine. Commands: `list`, `start`, `advance`, `stabilize`, `status`, `return`, `cancel`. Tracks active book, kind, world, anchor, intention, guide, depth, return count, Nothing degradation, souvenir due, and archive path in `players/[name]-book-jump.json`. Writes returned jumps to `memory/book-jumps/` and logs to `logs/book-jumps.jsonl`. Starting costs 1 Belief; a complete return grants +6 Belief, or +18 for *The Wonder Compass* memory jump. Must be run before prose for any start/continue/stabilize/return flow. |
...
| `scripts/penny-press.py` | Penny Blackletter / Press Page | Press & Peculiar Commerce desk. Commands: `init`, `scan [player]`, `proposal [player]`, `make [player] --kind <type> --topic "<topic>"`, `autonomous [player]`, `queue`, `approve <id>`, `reject <id>`, `revise <id>`, and `status`. Reads Storybook, scene ledger, Bleed, support-guild notes, recent images, NPC research, Wonder Compass excerpts, market research, editorial strategy, consent queue, social ledger, and feature/capability lines. Writes Press Packets to `memory/publishing/press-packets/`, structured draft candidates to `memory/publishing/candidates/`, platform briefs to `memory/publishing/briefs/`, proposals to `memory/publishing/proposals/`, optional local carousel/post images to `memory/publishing/images/`, the consent queue to `memory/publishing/consent-queue.json`, activity/performance memory to `memory/publishing/social-ledger.jsonl`, and logs to `logs/publishing/penny-press.jsonl`. Supported `--kind` values include `x-post`, `x-thread`, `instagram-carousel`, `tiktok-carousel`, `youtube-short`, `youtube-long`, `reddit-post`, `reddit-comment`, `patreon-post`, and `content-menu`. `make --generate-images` calls local Draw Things for carousel/hero art. Optional `--send` delivers review artifacts to Telegram as files; if Telegram document upload fails, Penny sends a text fallback with the local artifact path. Drafts only; never auto-posts. |
| `scripts/goldwater.py` | Professor Bastion Goldweaver / Applied Abundance | Product and ethical-revenue desk. Reads publishing memory, Storybook/Press output, Wonder Compass material, Patreon and product notes, market context, and support constraints; writes product/revenue briefs and one-small-action plans under `memory/publishing/` so monetization stays specific, humane, and shippable. |
| `scripts/press-abundance.py` | Penny + Goldweaver daily council | One synthesis call for Penny Blackletter and Professor Bastion Goldweaver. Reads the Listening Desk market brief, Penny's content pipeline, Goldweaver's product strategy, Storybook/Bleed/NPC research signals, and Wonder Compass/Doobaleedoos goals; writes a daily Press & Abundance meeting under `memory/publishing/press-abundance/`, logs to `logs/publishing/press-abundance.jsonl`, renders a storybook-journal PDF through `journal_artifact.py`, and can send the PDF to Telegram. Uses `openclaw/gpt55` for quality; no multiple-call simulated meeting. |
| `scripts/market-research.py` | Daily Listening Desk cron | Shared market/audience research for Penny and Professor Goldweaver. Fetches public signals from unauthenticated lanes where available (Reddit, Bluesky public search when reachable, Hacker News / Algolia), records source failures honestly, synthesizes struggles, borrowed language, platform fit, community etiquette, content openings, offer openings, and cringe traps through the OpenClaw `openclaw/gpt55` route. Writes `memory/publishing/market-research/daily/YYYY-MM-DD.md`, `memory/publishing/market-research/latest.md`, compact raw source JSON under `memory/publishing/market-research/raw/`, renders a storybook-journal PDF, sends it to Telegram when requested, and logs to `logs/publishing/market-research.jsonl`. Installed daily before the Penny/Goldweaver council so both desks share one audience map instead of running separate conflicting research. Commands: `daily [--no-fetch] [--no-llm]` and `status`. |
...
| `scripts/narrative_sim.py` | Internal (called by `world-pulse.py`) | Living-world simulation brain. Reads `lore/world-register.md`, thread state, recent action memory, character lore, and talisman pressure. Belief is narrative weight: high-Belief entities are selected more often and can bend more, but low-Belief entities still have a chance to matter. The internal action grammar is intentionally tiny: `take_action` (costs 1 Belief and leaves a trace), `invest_belief` (costs Belief and strengthens a target/thread/talisman), and `attack_belief` (costs Belief and weakens a target). Emits bespoke actor/thread-specific narrative vignettes with `visible_trace`, hidden mechanical ledger, Belief movement, and prose accounting instead of boilerplate templates; records recent trace signatures to avoid repetition; produces talisman intents, app pressures, and action lifecycle records that feed future scenes. |
...
| `scripts/ledger-faculty.py` | Support faculty / Gimble | Finance support loop for Gimble of the Errata Registry. Commands: `init`, `status`, `money-weather`, `weekly-audit`, `adventure-permission`, `question`, `brief`, and daily sync flows. Maintains `players/bj-ledger-chart.md`, `players/bj-ledger-log.jsonl`, and Actual Budget configuration. Reads fresh Actual Budget/SimpleFIN data through the local Actual API helper, supports auto-categorization, plain Telegram budget notes, and safe next-action advice. Daily sync and budget notes are written as Gimble's personal Errata Registry notes while preserving clear factual ledger data and insights. Never performs bank login or moves money. |
| `scripts/bellkeeper.py` | Support faculty / Bellkeeper | Proactive day-reading loop for Bellkeeper Elian Quill. Commands: `init`, `today`, and `status`. Maintains `players/bj-bellkeeper-chart.md`, `players/bj-bellkeeper-log.jsonl`, `config/proactive-offices.json`, and Today’s Page cards under `memory/support-faculty/bellkeeper/`. Reads the Enchantify Academy iCalendar feed, Apple Calendar summaries when available, HEARTBEAT, and support-character traces to name the day’s shape, one friction point, one tiny invitation, support on call, and tonight’s scrap. Calendar/reminder writes are proposal-only until the player explicitly approves. |
| `scripts/actual-api.mjs` | Internal / Gimble | Local Actual Budget API bridge. Used by Gimble, pulse, and Mission Control to read balances, accounts, recent transactions, categories, and sync state from the local Actual Budget server/cache. Also supports category setup and summary commands. Secrets live outside repo under `~/.openclaw/secrets/`. |
| `scripts/support_insights.py` | Support faculty / internal | Deterministic cross-domain insight engine shared by Vellum, Inkrest, Gimble, and the Support Guild. Reads Heartbeat, fuel log, Inkrest mood log, Vellum log, Gimble/Actual Budget summaries, and support memory; returns bounded pattern rows with title, detail, confidence, and next action. Used to prevent support messages from becoming stale balance/status recitations. |
| `scripts/support-faculty.py` | Cron + Telegram replies | Daily support layer for Dr. Selene Inkrest and Dr. Elowen Vellum. Commands include `inkrest-checkin`, `inkrest-route`, `inkrest-record`, `vellum-brief`, `synthesize`, and `research`. Maintains `players/bj-inkrest-pending.json`, `players/bj-inkrest-log.jsonl`, `players/bj-vellum-log.jsonl`, `players/bj-support-memory.json`, and weekly research/experiment briefs under `memory/support-faculty/research/`. Routes one-word mood replies safely, records explicit phrases like `Log for Dr Inkrest: ...` even without a pending prompt, reads Vellum/therapy charts and fuel/health signals, and keeps support work out of ordinary NPC research. Vellum's cron brief is now delivered as Refectory Marginalia from Dr. Elowen Vellum, with data and insights folded into character voice. |
| `scripts/support-guild.py` | Daily support guild cron | One-call council for Dr. Vellum, Dr. Inkrest, Gimble, Penny, Bellkeeper, and Professor Goldweaver. Reads latest mood, fuel, health, ledger, calendar/day-shape, press/market work, heartbeat, and support memory; writes a detailed cross-domain meeting artifact under `memory/support-faculty/guild/`; logs to `logs/support-faculty/guild.jsonl`; updates `players/bj-support-memory.json`; renders a storybook-journal PDF through `journal_artifact.py`; optionally sends Telegram summary plus the PDF/document artifact. Designed to be useful without multiple LLM calls. |
| `scripts/storybook.py` | Nightly Storybook Page cron / manual | Generates The Book of You, the player's illustrated storybook of lived Enchantify days. `daily [player] [--date YYYY-MM-DD] [--send] [--silent]` gathers scenes, a mix of play and hourly images, diary, Heartbeat, support faculty, ledger, simulations, Bleed ripples, anchors, outreach, Compass/Enchantment traces; writes Markdown, HTML, optional PDF, index, and `logs/storybook.jsonl`; sends the PDF through Telegram when `--send` is used, falling back to the HTML document only if PDF rendering is unavailable. `monthly [player] [--month YYYY-MM] [--send]` binds the month of daily pages into one collected PDF with an intro page, themes, highlights, and daily page links. |
| `scripts/journal_artifact.py` | Shared artifact renderer | Storybook-journal renderer for support, research, market, guild, press/abundance, and other living-document PDFs. Produces parchment/stamped/marginalia-style HTML and PDF artifacts with optional custom image plates; supports Chrome/wkhtmltopdf/reportlab-style fallbacks and Telegram document delivery callers. Used to make faculty briefs feel like pages from the Book rather than generic markdown exports. |
...
| `scripts/thread-closure.py` | Ending Pages / thread archive | Creates closure artifacts for resolved story threads. Reads current thread state, recent scenes/simulation/Bleed/context where available, writes a mini-story archive of what happened, renders a storybook-journal PDF through `journal_artifact.py`, and can push the PDF through Telegram. Used so a thread ending becomes a kept Page rather than only a row move. |
| `scripts/npc-research.py` | Living-world research / scheduled notes | NPC researches a topic from their Unwritten Interest and delivers findings. Uses local OpenClaw gateway (`NPC_RESEARCH_MODEL`, timeout) instead of spawning `openclaw agent`; `--model-smoke` verifies the route. Selection now includes any world-register NPC with Belief ≥ 8 and no cooldown, weighted by Belief, Unwritten Interest fit, relationship, core-cast bonus, and recent-research penalties. Character lookup canonicalizes titles, parentheticals, aliases, and compact lore rows. Each generated dispatch receives a compact continuity packet from recent play scenes, latest Book of You pages, open Bleed hooks, sender-specific entity memory, outreach replies, and prior research history. `logs/npc-research-history.jsonl` and `memory/npc-research/history/[character].md` prevent exact-topic loops; `--backfill-history` imports old notes, and each new dispatch receives a rotating required angle so it must add something new. The note must include at least one natural line tying the research back to bj, previous interactions, Academy events, or public Bleed stories. `--npc "Name"` forces an NPC and bypasses cooldown intentionally; `--dry-run` previews; `--no-print` skips CUPS; `--no-icloud` skips Notes; `--no-letter-image` skips Draw Things ornament; `--preview-letter memory/npc-research/[slug]-[date].md` renders an existing note as a styled letter without delivery. Writes to `memory/npc-research/[slug]-[date].md`, `memory/npc-research/letters/[slug]-[date].html`, optional `letter-images/[slug]-[date].png`, and a PostScript fallback for printing when needed. Telegram and iCloud delivery have explicit timeouts. Queues tick-queue narrative seed. Deducts 3 Belief. 72-hour cooldown except forced runs; cache is `config/npc-research-cache.json`. |
...
| `scripts/mission-control.py` | pulse.py (every 15 min) + manual | Generates `hooks/mission-control.html` — the Story-Field Journal live HTML folio. Shows: story threads, Page Contract, Scene Director/scene construction rails, full world-register entity sections, talismans, app actions, anchors, images, schedule, heartbeat, forecast, narrative health, cron status, and recent Bleed issues. Entity rows include source register section and click-through details. Thread status and phase are read from Active Threads Notes column. Run manually: `python3 scripts/mission-control.py`. Auto-regenerated on every pulse run. |
...
| `scripts/bleed.py` | Daily 6 PM cron (`0 18 * * *`) | Publishes The Bleed — the Academy student newspaper. Reads world-register.md, threads.md, tick-queue.md, HEARTBEAT.md, players/[name].md, app-register.md, fuel/health data, and prior coverage. Uses local OpenClaw gateway (`BLEED_MODEL`, timeout) with `--model-smoke`; on gateway failure uses a deterministic validated fallback issue instead of skipping. Gossip is validated/repaired into signed corridor-source items from actual characters, never W.E. writing about himself. Exchange ticker text is normalized into structured rows. Dr. Vellum's Desk replaces the old Provisions Log with longevity guidance. Feature images are character-first field-journal portraits. After publication, `record_bleed_ripples()` writes public-pressure records to `logs/bleed-ripples.jsonl`, then `scripts/bleed-memory.py update` writes latest-issue awareness into entity memory so articles feed back into play. Sends Telegram edition and prints through guarded CUPS path. `--force` regenerates. |
| `scripts/bleed-memory.py` | After Bleed publication / manual | Deterministic bridge from the latest Bleed issue to character memory. Reads `logs/bleed-ripples.jsonl`, detects named entities and section-watchers, writes the current issue map to `memory/entities/bleed-awareness.json`, and re-renders affected `memory/entities/*.md` cards with a `Latest Bleed Notice` section. `story-context.py` then surfaces those notices as `ENTITY_MEMORY` so characters can reference today's paper without another LLM call. Commands: `update`, `status`. |
...
| `scripts/mechanics-preflight.py` | Labyrinth (before every active-play reply) | Active-play mechanics gate. Reads `mechanics/mechanics_state.py` and outputs compact obligations: once-daily Compass eligibility, recovery Enchantment status, scene-contract Enchantment handoff, dice pressure, consecutive declines, Belief band. At Belief 60 or below, formal Compass/Enchantment offers are treated as fun recovery routes back into investment power, not last-ditch interventions. Exit 0 normally; exit 1 in `--strict` mode when pressure must not be ignored. `--json` for machine-readable output. Run within 15 minutes of any scene or `play_scene.py` will refuse. |
...
| `scripts/scene_packet_builder.py` | Internal (called by play_scene.py) | Builds a `ScenePacket` from the current game state without replacing the story spine. Reads Page tool posture from `page-contract.py` / `scene-contract.py`, applies cooldowns via `memory/page-tool-state.json`, and emits contextual sequences rather than legacy intensity defaults. Image prompts are character-first field-journal portraits: face, expression, posture, hands, clothing, one meaningful object; architecture is only background wash. Usage: `python3 scripts/scene_packet_builder.py [player] --text-file /tmp/scene.txt --voice-file /tmp/voice.txt --out /tmp/scene.json`. |
| `scripts/scene_conductor.py` | Internal (called by play_scene.py → scene_packet_builder) | Multi-modal scene orchestrator. Takes one `ScenePacket` JSON and fans out across adapters in Page-selected sequence: text, voice, image, lights, MusicGen, Spotify, printer, wallpaper, and private app actions. Buffered delivery on Telegram (media prepared first, then flushed in order). Private app actions use safe drivers; public/social actions remain consent-gated. Writes per-scene JSON payloads and run records to `tmp/scene-outbox/`. Current archive art style is the magical field-journal manuscript look. Default routing model: `openai-codex/gpt-5.4-mini`. `--emit-example` prints a full example packet. |
...
| `scripts/story-context.py` | Before active scene writing | Synthesizes the compact long-memory braid: continuity threads, quiet-life threads, recent realized scenes, active threads, NPC research, open simulation actions, Bleed ripples, narrative obligations, and what to avoid. Designed so smaller models can preserve continuity without reading the whole world. |
| `scripts/entity_memory.py` | Internal (called by world-pulse.py, outreach-memory.py, record_scene_run.py, bleed-memory.py; read by story-context.py) | Persistent per-entity memory rail. Records simulation actions, Belief shifts, outreach, BJ replies, delivered live-scene appearances, and latest Bleed awareness into `logs/entity-memory.jsonl`, `memory/entities/bleed-awareness.json`, and compact readable cards in `memory/entities/*.md`. `story-context.py` surfaces relevant memories as `ENTITY_MEMORY`; active play should treat those as hard continuity for trust, callbacks, grudges, public rumors, avoided repetition, and relationship texture. |
| `scripts/scene-contract.py` | Before and after active scene drafting | Emits MODE / DRAMA_BUDGET / PAGE / LONG_MEMORY / QUIET_LIFE / grounding / continuity / classroom / tool-packet / choice-contract constraints. It surfaces once-daily Compass opportunities and scene-appropriate Enchantment opportunities from `mechanics_state.py`, class context, concrete objects/clues, Nothing pressure, unresolved NPC traces, and quiet-life wonder. `--json` exposes current scene construction rails for Story-Field Journal. `--validate-scene` checks grounding, mode fit, class compression, Page fit, formal mechanic offers, and Rule-of-Three-related expectations before delivery. |
...
| `scripts/check-runtime.py` | Manual/runtime smoke suite | Runs the reliability suite: lean root, AGENTS size, syntax, mechanics/speaker preflight, story context/progress, page contracts, classroom validation, scene contract/choices/fullness, Page tool packet builder, character-first image prompts, closeout schema, health reader, Bleed fallback/Vellum/ripples, cron steward, housekeeping, Draw Things keepalive, support-faculty routing, fae ledger, thread lifecycle/closure, actor-composed simulation variety, NPC research path, outreach path, live-scene repair, and live-scene dry run. |
...
| `scripts/send_academy_dispatch.py` | 4-hour simulation cron (after world-pulse.py) | Sends the Academy simulation dispatch through `multi_voice_tts.py`. Checks `config/session-active.lock`: fresh locks skip and log; locks older than 18h are treated as stale, cleared, and dispatch proceeds. Builds a narrative `Between Bells` vignette from the latest simulation prose plus a small ledger line. It no longer uses the old `NPC Decisions / Story Threads / Environment / One-Liner` report format. |
...
| `scripts/housekeeping.py` | Nightly 4:35 AM cron | Conservative storage steward. Keeps the latest 3 OpenClaw dated backups, trims transient scene-outbox files older than 2 days and old `/tmp/enchantify-*` helper backups older than 14 days, logs to `logs/housekeeping/last-run.json`, and never touches models, Draw Things assets, Bleed issues, diaries, letters, wallpapers, or archives. `--dry-run` previews. |
| `scripts/drawthings-keepalive.py` | Hourly cron (`7 * * * *`) | Draw Things watchdog. Pings the local Draw Things API `/sdapi/v1/options`, opens `Draw Things.app` if needed, optionally generates one small character-focused Enchantify field-journal image through the local Qwen/Draw Things path, and keeps only the latest 24 keepalive images in `logs/drawthings-keepalive/`. Subject selection can pull weighted characters from visual canon/narrative weight plus occasional Academy or Library scenes. Telegram captions are deterministic and name what/who is illustrated without an LLM call. |
| `mechanics/mechanics_state.py` | Imported by mechanics-preflight.py, play_scene.py, run-live-scene.py | Central mechanics state module. Reads and writes the mechanics state JSON for a given player. Provides: `get_mechanics_state()`, `get_preflight_status()`, `record_mechanic_event()`. Tracks daily Compass/Enchantment offer counts, completion dates, active proof gates, Belief band, dice guidance, and preflight freshness. Current cadence: once-daily Compass offer at Belief ≤ 60 if not completed today; up to two routine Enchantment offers at Belief ≤ 60 if none completed today. All scripts that gate on mechanics freshness import this module. |
| `scripts/session-entry.py` | Session open (Step 0b — before first narrative reply) | **Canonical entry point.** Determines how the returning player enters the Labyrinth based on time away: `in_media_res` (<1h, scene still warm), `dorm_brief` (1–8h, one or two things to notice), `dorm_full` (>8h, full dorm arrival with dynamic objects). Dorm returns now hand off a Dorm Page state through `memory/session-entry-current.json`, so `page-contract.py` / `scene-contract.py` choose the Dorm Page instead of treating return as generic Slice of Life. Reads `players/[name]-session.json` for away hours (written by `clear-lock.py`). Calls `schedule.py` for the SCHEDULE CONTEXT block and `scene-director.py` for the Director's Slate. Outputs ENTRY_MODE directive + SCHEDULE CONTEXT + Director's Slate in one pass. Dynamic objects translate tick-queue and thread state into physical dorm evidence — a note under the door, a book not where it was left, edged light. |
...
| `scripts/class-lecture.py` | Labyrinth (Wonder Compass class sessions) | Multi-turn classroom lecture state manager. Does not send messages; prepares class-scene directives for the Labyrinth to narrate. Modes: `--attend` (mark attendance for current block), `--advance` (step to next lesson), `--status` (show current lesson state), `--json` (machine output). Reads lesson content from `config/class-curriculum.json`. Tracks lesson progress only when the player attends. Directive now forbids compressed class summaries and requires one teachable segment at a time: professor dialogue, concrete concept, classmates, practice shape, and continuation options. Supports Wonder Compass book cross-references. |
| `scripts/compass-run.py` | Labyrinth (formal Compass Runs) | Deterministic Wonder Compass state machine. Commands include `start`, `answer`, `status`, `complete-west`, and `cancel`. Stores active state in `players/[player]-compass-run.json`; calibrates North/East/South/West prompts from Chapter 5, `lore/compass-run.md`, HEARTBEAT, weather, time, location, steps, work pressure, mood, schedule, and fuel; writes the souvenir through the proper souvenir/card path; updates mechanics and Belief. Compass Runs must never be advanced or completed by prose alone. |
...
│   ├── npc-research/             ← NPC research notes, styled letter HTML/PS artifacts, optional letter-images
...
│   ├── bleed-ripples.jsonl       ← Public-pressure records created by published Bleed articles
...
| World Belief Pulse | `30 */4 * * *` | `arc-tick.py && tick.py && world-pulse.py && send_academy_dispatch.py` — advances arc day/phase, stirs world-register entities, emits tick-queue seeds, may trigger bounded NPC research, then sends a narrative Between Bells dispatch if no fresh session lock is present |
...
**🎨 Draw Things (local image generation):** `scripts/drawthings_scene.py` calls the Draw Things API at `localhost:8080`. Generates scene images for cinematic and ritual intensity levels, living wallpapers, Bleed feature illustrations, and optional NPC research letter ornaments without any external API call. Configurable: prompt, width, height, steps, CFG scale. Current default style across generated imagery is a magical field-journal manuscript page: sparse pen-and-ink linework, lush watercolor washes, aged parchment, visible paper grain, soft ink bleed, watercolor blooms, lush handwritten marginalia, visible library stamps, wax seals, labels, tabs, arrows, annotations, archival overlays, and selective pops of color. Page furniture should be abundant and integral, not timid decoration. Called by `scene_conductor.py`, `wallpaper.py`, `bleed.py`, and `npc-research.py`.
...
**Consent and visibility model:** Private app actions (Spotify, Notes, Calendar, Reminders, Obsidian, Telegram) may execute without consent, but they are no longer invisible: completed/failed app talisman actions send a concise Telegram notice and log `telegram_sent` in `logs/pact-actions.jsonl`. Social media posts (Moltbook, Bluesky, Reddit, iMessage) require consent at Dominated/Sovereign. X/Twitter requires consent at all tiers. Consent-required actions write `logs/pending-consents.jsonl`, send Telegram notices with the consent ID and actual proposal, and appear as `[CONSENT REQUIRED]` in tick-queue/Mission Control. When a LLM spec is generated for a consent-required action, the preview shown in the consent prompt contains the actual LLM-generated content — not a generic template.
...
| **FUEL** | **Dr. Vellum's Desk** — right-rail longevity column by Dr. Elowen Vellum. Uses logged food/fuel, health/vitality signals, and practical longevity framing to give precise, non-shaming rest-of-day advice. It may mention meals, but it is no longer only a food column. |

**Data sources:** `lore/world-register.md` (entity standings + leading talisman + chapter NPCs), `lore/threads.md` (thread summary + market odds), `memory/tick-queue.md` (simulation activity), `HEARTBEAT.md` (weather, health), `players/[name].md` (story log, quests, Compass history), `lore/app-register.md` (war analytics), `scripts/fuel-log.txt` and `scripts/food_log.py` nutrition entries (fuel/longevity context).
...
**The Bleed feeds back.** The player's Labyrinth of Stories book does not need separate schedule/thread/arc/war sections — the newspaper covers them all with voice and journalism. The Labyrinth covers the current moment in session; The Bleed covers the world state. After publication, `record_bleed_ripples()` writes public-pressure records to `logs/bleed-ripples.jsonl`. `scripts/bleed-memory.py` then converts the latest issue into per-character awareness, overwriting the daily Bleed notice map so today's paper can appear in dialogue as fresh corridor talk without accumulating stale lore. Later scenes read those ripples and notices as rumor, reputation pressure, market offers, practical advice, and public interpretation. These are not objective canon rewrites; they are what the Academy now believes, argues about, misreads, and acts on.
...
run-live-scene.py            ├─ spotify ───────────────→ pact-driver → Spotify
        ...
        ├─ wallpaper ─────────────→ macOS desktop
        └─ app_actions ───────────→ private app drivers / consent gates
        ▼
  play_scene.py              Intensity sequences:
        │                      quiet    → text, voice
        ▼                      cinematic → text, image, voice
scene_packet_builder.py        Page tool posture chooses sequence by Page Type
        │                      example conflict → text, lights, image, voice, music, Spotify, wallpaper, app_actions
        │
        ▼
...
- Page tool posture decides which tools are allowed, preferred, rare, or forbidden for the current Page. This prevents global tool soup: Rest Pages stay gentle; Conflict Pages may use lights/MusicGen/wallpaper/app actions; Letter Pages may print or create notes; Enchantment Pages privilege photo/image proof.
- Image generation is character-first: prompts center named characters, faces, posture, hands, clothing, and meaningful objects; rooms/corridors remain faint background wash. Current visual style is sparse pen-and-ink linework, lush watercolor washes, aged parchment, visible paper grain, soft ink bleed, manuscript layout, lush marginalia/labels, visible library stamps, wax seals, tabs, arrows, archival overlays, and selective jewel-like color. Page furniture should be abundant and integral, not timid decoration.
...
- ✅ **Project image style unified** — SOUL.md, tutorial/spawn portrait instructions, Everything's Archive, scene images, wallpapers, and Bleed feature art now use the magical field-journal manuscript style: sparse pen-and-ink linework, lush watercolor on aged parchment, visible grain, ink bleed, watercolor blooms, lush handwritten marginalia, visible library stamps, wax seals, labels, tabs, arrows, archival overlays, and selective color.
...
- ✅ **Director's Slate: Return care in NOTHING** — `layer_nothing()` reads `**Last run:**` from the player file's Compass Run History and appends a warm RETURN CARE modifier rather than a guilt/penalty cue. Thresholds: 0–2 days: no note. 3–5 days: outside-world glimmer gently. 6–10 days: outside-world magic is available when wanted. 10+ days: warmly offer a tiny Compass Run after welcoming them back. No Compass Run on record: offer the first one as an invitation, not a correction.
...
- ✅ **Belief economy redesign** — Starting Belief 30. Real costs for dice failures (by difficulty), Nothing encounters (−3/−5/−10 tiers), session gap decay, and decline tracking. Enchantment rebalanced to 3/+9 (risk/reward meaningful). Initial offer thresholds were later superseded by the v16.7 recovery cadence.
...
### v16.7.0 — Storybook, Press, Memory & Offer Cadence (May 23, 2026)

- Added `scripts/bleed-memory.py`, a no-LLM bridge from the latest Bleed issue to per-entity memory. The latest issue now writes `memory/entities/bleed-awareness.json` and re-renders affected `memory/entities/*.md` cards with `Latest Bleed Notice`, so characters can reference current Bleed stories in play without stale accumulation.
- Extended `scripts/entity_memory.py` and `story-context.py` so entity memory now includes simulation actions, Belief shifts, outreach/replies, delivered scene appearances, and latest Bleed notices. Relevant `ENTITY_MEMORY` can now surface public newspaper talk, relationship continuity, and offscreen memories together.
- Added NPC research history: `logs/npc-research-history.jsonl`, `memory/npc-research/history/[character].md`, `--backfill-history`, runtime-noise cleanup, and rotating required research angles. NPC research prompts now see prior dispatches and must find a fresh subtopic/angle instead of repeating the same research shelf.
- Fixed NPC research cooldown logic so the 72-hour per-character cooldown is actually enforced unless the run is forced.
- Raised the real-magic offer cadence. `mechanics/mechanics_state.py`, `mechanics-preflight.py`, `scene-contract.py`, and `AGENTS.md` now treat Belief 60 or below as recovery range: one formal Compass Run offer per day if not completed, and up to two script-backed Enchantment opportunities per day when the scene has a real object/clue/class/quiet-life opening. Fixed a date bug where an old Enchantment completion could suppress future offers.
- Added Professor Bastion Goldweaver as a full support character: Professor of Applied Abundance, ethical product/Patreon/merchandise/offer strategist, and Penny Blackletter's strategic partner.
- Added `scripts/goldwater.py` and `scripts/press-abundance.py`. Penny + Goldweaver now share one daily strategy synthesis, use the market brief and game/story outputs, write meeting memory, render a storybook-journal PDF, and can push the artifact to Telegram.
- Added/installed the shared Listening Desk: `scripts/market-research.py` and `install-market-research-cron.py`. It gathers public audience/platform signals for Penny and Goldweaver, uses `openclaw/gpt55`, writes markdown/raw logs, renders a storybook-journal PDF, and feeds both content and product strategy.
- Upgraded `scripts/journal_artifact.py` into the shared storybook-journal PDF renderer for support guild meetings, market briefs, support research, research letters, Press/Abundance, and other faculty artifacts.
- Upgraded the Support Guild to include Dr. Vellum, Dr. Inkrest, Gimble, Penny, Bellkeeper Elian Quill, and Professor Goldweaver, with stronger cross-domain synthesis across fuel, health, mood, ledger, calendar/day-shape, publishing, market research, and current support memory.
- Added Book of You monthly binding: `scripts/storybook.py monthly` collects daily pages for a month into a single PDF with intro page, themes, highlights, and daily links. Daily pages now prefer play images while still mixing in hourly generated images.
- Added thread closure artifact support through `scripts/thread-closure.py`: closing a thread can create a mini-story archive/artifact and Telegram-ready PDF instead of only moving rows out of active state.
- Updated Mission Control / Story-Field Journal surfaces across the day for Inkrest, Penny, Bellkeeper, Current Page, Ledger/Gimble, Storybook/Compass/Book Jump visibility, Scene Director freshness, and Academy environment forecasting.
- Hardened image/artifact style: generated journal PDFs, research letters, support briefs, market briefs, and wallpaper/scene images share the lush field-journal/storybook look with stronger character descriptions, player identity, custom image plates, marginalia, stamps, and watercolor wash cues.

### v16.5.0 — Support Guild, Wider Research & Fresh Memory Rails (May 17, 2026)

- Added the daily Support Guild: `scripts/support-guild.py` convenes Dr. Vellum, Dr. Inkrest, and Gimble in one LLM call, writes a full meeting artifact to `memory/support-faculty/guild/`, logs to `logs/support-faculty/guild.jsonl`, updates `players/bj-support-memory.json`, and can deliver both Telegram summary and markdown file.
- Added `scripts/support_insights.py`, a shared cross-domain insight engine for support faculty. Vellum briefs, Gimble budget notes/daily sync/audits, support synthesis, and Support Guild meetings now receive deterministic pattern rows across Heartbeat, fuel, mood, ledger, and support memory before speaking in character.
- Added The Book of You: `scripts/storybook.py` creates a daily Storybook Page as Markdown, HTML, and PDF, indexes it, logs it, and can deliver the PDF through Telegram. It uses the day's real/game signals and saved illustrations as a literary life chapter rather than a report.
- Hardened Dr. Inkrest mood memory. One-word replies still route through pending prompts, but explicit phrases like `Log for Dr Inkrest: awake and productive`, `Inkrest: anxious`, and `Mood: tired but okay` now record even without a pending prompt. The agent must only confirm mood logging when `support-faculty.py` reports a recorded write.
- Updated support synthesis and guild briefings to treat `latest_mood`, `latest_prompt`, unanswered prompt count, and pending state as authoritative, preventing old mood data from being presented as current.
- Promoted Gimble from readiness to live Actual Budget / SimpleFIN support: daily sync, auto-categorization, fresh balances, recent transactions, plain budget notes, Mission Control ledger data, Heartbeat finance data, and Telegram delivery all flow through the local Actual API bridge.
- Broadened NPC research. `npc-research.py` no longer hard-gates research to core characters or relationship ≥ 25. Any world-register NPC with Belief ≥ 8 and no cooldown can be selected, weighted by Belief, Unwritten Interest fit, relationship, core-cast bonus, and recent-research penalties.
- Hardened character lookup for NPC research and simulation so parentheticals, titles, aliases, and compact lore rows resolve back to `lore/characters.md` instead of silently dropping dossiers.
- Enriched Unwritten Interests across the roster, including witchcraft and magick, chaos magick, Bungie Marathon, The Elder Scrolls Online, restaurants, cooking, ethnic cooking, historical cooking, weather witchcraft, kitchen witchcraft, temple food, seafood foodways, and civic folklore.
- Added simulation freshness rails: `narrative_sim.py` now applies global recent-actor penalties and freshness bonuses so the same entities and repeated action shapes do not keep surfacing word-for-word.
- Documented deterministic Compass Runs, including Chapter 5 calibration, fuel/mood/heartbeat/weather/schedule context, stateful North/East/South/West progression, souvenir card writing, and the rule that Compass completion cannot happen in prose alone.
- Documented recent delivery and visual hardening: duplicate fallback tails are stripped, valid text delivery suppresses fallback, player gender and character descriptions are injected into image prompts, hourly Draw Things captions name their subject, and Vellum's Bleed column treats current fuel data as authoritative.
- Rebuilt the simulation surface: four-hour simulation summaries now use `Between Bells` narrative vignettes plus a small ledger instead of `NPC Decisions / Story Threads / Environment / One-Liner`. The simulation uses only `take_action`, `invest_belief`, and `attack_belief` internally, and every selected action costs Belief.
- Added deterministic Book Jumping: `scripts/book-jump.py` now governs starting, advancing, stabilizing, returning, canceling, Belief cost/reward, Nothing degradation, souvenir due, and archive artifacts so Book Jumps are playable for smaller models instead of freeform lore.

### v16.4.0 — Contextual Pages, Bleed Ripples & Stewardship Surfaces (May 12, 2026)

- Added Page tool posture as a first-class mechanic. Each Page now defines preferred, allowed, rare, and forbidden tools plus intrusion level, cooldowns, audio roles, default sequence, and triggers. This turns global capabilities into contextual affordances.
- Rewired `scene_packet_builder.py` and `scene_conductor.py` so Page contracts can actually invoke lights, Spotify, local MusicGen, wallpaper, private app actions, printing, image generation, and voice in the right moments instead of relying on legacy intensity buckets.
- Added wallpaper and app-action adapters to the scene conductor. Private app actions can execute through safe local drivers; public/social actions remain consent-gated.
- Updated image generation policy across scene packets and The Bleed: images should center characters, faces, posture, hands, clothing, and meaningful objects, with architecture as faint background only.
- Added Story-Field Journal visibility for Page Contract and Scene Director/scene-construction rails, including current location, mode, drama budget, continuity anchor, tool sequence, must/must-not rules, open simulation traces, and narrative repair duties.
- Added Bleed Ripples: The Bleed now writes `logs/bleed-ripples.jsonl` after publication, and `story-context.py` / `scene-director.py` read those ripples as public interpretation, rumor pressure, practical advice, market offers, and social weather.
- Upgraded Dr. Vellum's Desk: the old Provisions Log is now a longevity physician/dietician column using logged fuel, health signals, and practical longevity research without inventing meals or shaming.
- Added BJ's Vellum Chart at `players/bj-vellum-chart.md`: a durable scaffold for bloodwork, home blood pressure, medications, supplements, allergies, conditions, training limits, goals, and active longevity experiments. Vellum may recommend supplements, exercise protocols, and daily experiments, but must label evidence strength, risks/interactions, missing context, and doctor/pharmacist questions when relevant.
- Added `scripts/vellum-chart.py`, the structured update path for Vellum's chart. Use `show`, `known`, `medical`, `bp`, `lab`, `supplement`, `experiment`, and `question` instead of direct markdown edits. Examples: `python3 scripts/vellum-chart.py bp 128 82 --pulse 74 --context "after coffee"`; `python3 scripts/vellum-chart.py lab A1C 5.4 --unit % --date 2026-05-10`; `python3 scripts/vellum-chart.py supplement "creatine monohydrate" --dose 5g --frequency daily --why "strength/longevity experiment"`.
- Added a dedicated Story-Field Journal Vellum tab. Mission Control now surfaces latest health signals from `HEARTBEAT.md`, recent fuel-log nutrition entries, and the structured Vellum Chart summary so BJ can see what Dr. Vellum actually knows before she gives advice.
- Added Dr. Selene Inkrest, Academy Narrative Therapist / Keeper of Difficult Pages. She uses narrative therapy as the spine, depth psychology as atmosphere, and practical grounding/ACT/CBT/parts language as tools. Her Unwritten Interest is consciousness and brain studies as they relate to BJ: predictive processing, default mode network, attention, rumination, neuroplasticity, memory, daydreaming, imagination, selfhood, medication effects on dreaming/sleep, nervous-system regulation, narrative identity, and how consciousness becomes story. She reads `players/bj-therapy-chart.md`, Vellum's chart, fuel log, HEARTBEAT, diary/daydream material, and recent scene context; she does not diagnose, replace care, or force catharsis.
- Added `scripts/therapy-chart.py` for structured therapy artifacts: `checkin`, `daydream`, `reauthor`, `question`, `frame`, `care`, and `show`. Added the Dr. Inkrest / Difficult Page type so therapy/support scenes have consent-first rails, low intrusion, and chart-backed artifacts.
- Added Dr. Inkrest Office Hours to schedule/calendar: Tuesday and Thursday at 9:30 PM in the Reauthoring Rooms. `scripts/build-academy-calendar.py` now writes those recurring events into `hooks/enchantify_schedule.ics`, and Story-Field Journal schedule cards show support hours.
- Hardened classroom play: `class-lecture.py` and `scene-contract.py` now reject compressed class summaries and keep lessons multi-turn, attended, professor-led, classmate-present, and practice-oriented.
- Hardened app talisman visibility: completed/failed pact app actions now send Telegram notices; consent-required actions send Telegram requests with consent ID and actual proposal; Mission Control records whether Telegram was sent.
- Added simulation variety checks so repeated Academy Daily Life traces like identical gate-holding actions are avoided by runtime tests.
...
- Added `scripts/page-contract.py`, a deterministic Page chooser for smaller models. It selects Slice of Life, Conflict, Enchantment, Wonder Compass, Letter, Anchor, Rest, Dr. Vellum / Body Marginalia, Dr. Inkrest / Difficult, Archive, or Bleed, then returns allowed systems, forbidden systems, invitation, closure, artifact due, tool posture, and recommended scene mode/drama budget.
...
- ⏳ Enchantify app / story-journal UI refinement (ink sketches, animations; do not use the old Glass name)
